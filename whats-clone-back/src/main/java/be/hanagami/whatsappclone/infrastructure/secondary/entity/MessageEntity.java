...
import be.hanagami.whatsappclone.infrastructure.secondary.entity.MessageEntityBuilder;
...
import be.hanagami.whatsappclone.messaging.domain.message.aggregate.MessageBuilder;
...
public class MessageEntity extends AbstractAuditingEntity<Long> {
    ...
    @Column(name = "publi_id", nullable = false, updatable = false)
    ...
    public static Message toDomain(MessageEntity messageEntity) {
        ...
        if (messageEntity.getType().equals(MessageType.TEXT)) {
            ...
            MessageContent content = new MessageContent(messageEntity.getText(), messageEntity.getType(),
                    messageMediaContent);
            messageBuilder.content(content);
        }
    ...
    public MessageEntity(Long id, UUID publicId, Instant sendTime, String text, MessageType type,
                         MessageSendState sendState, UserEntity sender, ConversationEntity conversation,
                         MessageContentBinaryEntity contentBinary) {
        this.id = id;
    ...
    @Override
    public int hashCode() {
        int result = Objects.hashCode(publicId);
        result = 31 * result + Objects.hashCode(sendTime);
        result = 31 * result + Objects.hashCode(text);
        result = 31 * result + Objects.hashCode(type);
        result = 31 * result + Objects.hashCode(sendState);
        return result;
    }
}
